<div class="row">
    <div class="col-sm-12">
        <form role="form" class="form-horizontal form-groups-bordered" action="<?php echo base_url(); ?>index.php?report/reprint_grn_returned_report" method="post" enctype="multipart/form-data">
                ...
                <div class="col-sm-6">
                    ...
                    <div class = "col-sm-9">
                        <select name="refno" class="form-control selectboxit" >
                            <?php 
                                $this->db->order_by("gnrno");
                                $this->db->group_by("gnrno");
                                $list = $this->db->get("goodsreturned")->result_array();
                                foreach($list as $item){ 
                                    $supname = $this->db->get_where("creditsuppliers",array('CustomerId'=>$item["sid"]))->row()->CustomerName;
                                    ?>
                                    <option value="<?php echo $item['gnrno']; ?>" <?php if ($item["gnrno"]==$refno) echo 'selected'; ?> ><?php echo $item['gnrno']."-".$supname."-".$item['date']."-".$item['invoice']; ?></option>        
                            <?php    } ?>
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    jQuery(window).load(function ()
    {
        ...
        $("#print-view").on("click",function(e){
            ...
            window.open("<?php echo base_url(); ?>"+"index.php?report/reprint_grn_returned_report/print_view/"+option);
        });
